    ...
    if padding_side == 'left':
        ...
        if pad_token_id is None:
            if attention_mask is not None:
                ...
                raise ValueError('Cannot handle batch sizes > 1 if no padding token is defined and no attention_mask is provided.')
            else:
                sequence_lengths = -1
        else:
            if input_ids is not None:
...
def _patch_sequence_classification(model, model_meta):
    ...
    # For some MLLM architectures (e.g. Qwen3.5), `transformers>=5` may still
    # try to mark tied weights as initialized during `from_pretrained` finalize.
    # Since seq_cls replaces lm_head with Identity, keeping tied-weight metadata
    # can trigger `Identity has no attribute weight`.
    if hasattr(model, 'config') and hasattr(model.config, 'tie_word_embeddings'):
        model.config.tie_word_embeddings = False
    if hasattr(model, '_tied_weights_keys'):
        model._tied_weights_keys = []
    if hasattr(model, '_dynamic_tied_weights_keys'):
...
def patch_automodel_for_sequence_classification(model_info=None,
    ...
    mark_tied_weights_as_initialized = PreTrainedModel.mark_tied_weights_as_initialized
    ...
    if patch_from_pretrained:
        ...
        def _new_mark_tied_weights_as_initialized(self, *args, **kwargs):
            try:
                return mark_tied_weights_as_initialized(self, *args, **kwargs)
            except AttributeError as e:
                if 'Identity has no attribute' in str(e) and 'weight' in str(e):
                    logger.warning('Ignore tied-weights initialization for seq_cls patched model: %s', e)
                    return
                raise

        PreTrainedModel.mark_tied_weights_as_initialized = _new_mark_tied_weights_as_initialized
