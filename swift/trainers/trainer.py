...
class Trainer(SwiftMixin, DataLoaderMixin, HfTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.compute_loss_func is not None and labels is not None:
            outputs = model(**inputs)
            loss = self.compute_loss_func(outputs, labels, inputs=inputs, num_items_in_batch=num_items_in_batch)
        else:
            loss, outputs = super().compute_loss(model, inputs, return_outputs=True)
